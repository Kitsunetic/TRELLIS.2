...
def read_vxz(file, num_threads: int = -1) -> Union[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    def worker(chunk_info):
        ...
        for k, v in chunk_info.items():
            if k in ["ptr", "idx"]:
                continue
            decompressed[k] = np.frombuffer(
                _decompress(chunk_data[v[0] : v[0] + v[1]], structure_data["compression"], structure_data["compression_level"]),
                dtype=np.uint8,
            )
    ...
    for info, chunk in zip(structure_data["chunks"], chunks):
        coord.append(
            chunk["coord"]
            + torch.tensor([[info["idx"][0] * chunk_size, info["idx"][1] * chunk_size, info["idx"][2] * chunk_size]]).int()
        )
