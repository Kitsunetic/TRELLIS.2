...
def textured_mesh_to_volumetric_attr(
    ...
    if voxel_size is not None:
        ...
        ), f"voxel_size must be a float, list, tuple, np.ndarray, or torch.Tensor, but got {type(voxel_size)}"
    ...
    if grid_size is not None:
        ...
        ), f"grid_size must be an int, list, tuple, np.ndarray, or torch.Tensor, but got {type(grid_size)}"
    ...
    for sid, (name, g) in tqdm(
        enumerate(scene.geometry.items()), total=len(scene.geometry), desc="Loading Scene", disable=not verbose
    ):
        if verbose:
            ...
            if g.visual.material.baseColorTexture is not None:
                print(
                    f"  Base color texture: {g.visual.material.baseColorTexture.size} {g.visual.material.baseColorTexture.mode}"
                )
            ...
            if g.visual.material.metallicRoughnessTexture is not None:
                print(
                    f"  Metallic roughness texture: {g.visual.material.metallicRoughnessTexture.size} {g.visual.material.metallicRoughnessTexture.mode}"
                )
        ...
        ), f"Only trimesh.visual.TextureVisuals is supported, but got {type(g.visual)}"
        assert isinstance(
            g.visual.material, trimesh.visual.material.PBRMaterial
        ), f"Only trimesh.visual.material.PBRMaterial is supported, but got {type(g.visual.material)}"
        ...
        uvs = (
            torch.tensor(g.visual.uv[g.faces], dtype=torch.float32)
            if g.visual.uv is not None
            else torch.zeros(g.triangles.shape[0], 3, 2, dtype=torch.float32)
        )  # [N, 3, 2]
        baseColorFactor = (
            torch.tensor(g.visual.material.baseColorFactor / 255, dtype=torch.float32)
            if g.visual.material.baseColorFactor is not None
            else torch.ones(3, dtype=torch.float32)
        )  # [3]
        baseColorTexture = (
            torch.tensor(np.array(g.visual.material.baseColorTexture.convert("RGBA"))[..., :3], dtype=torch.uint8)
            if g.visual.material.baseColorTexture is not None
            else torch.tensor([])
        )  # [H, W, 3]
        metallicFactor = g.visual.material.metallicFactor if g.visual.material.metallicFactor is not None else 1.0
        metallicTexture = (
            torch.tensor(np.array(g.visual.material.metallicRoughnessTexture.convert("RGB"))[..., 2], dtype=torch.uint8)
            if g.visual.material.metallicRoughnessTexture is not None
            else torch.tensor([])
        )  # [H, W]
        roughnessFactor = g.visual.material.roughnessFactor if g.visual.material.roughnessFactor is not None else 1.0
        roughnessTexture = (
            torch.tensor(np.array(g.visual.material.metallicRoughnessTexture.convert("RGB"))[..., 1], dtype=torch.uint8)
            if g.visual.material.metallicRoughnessTexture is not None
            else torch.tensor([])
        )  # [H, W]
        emissiveFactor = (
            torch.tensor(g.visual.material.emissiveFactor, dtype=torch.float32)
            if g.visual.material.emissiveFactor is not None
            else torch.zeros(3, dtype=torch.float32)
        )  # [3]
        emissiveTexture = (
            torch.tensor(np.array(g.visual.material.emissiveTexture.convert("RGB"))[..., :3], dtype=torch.uint8)
            if g.visual.material.emissiveTexture is not None
            else torch.tensor([])
        )  # [H, W, 3]
        ...
        alphaTexture = (
            torch.tensor(np.array(g.visual.material.baseColorTexture.convert("RGBA"))[..., 3], dtype=torch.uint8)
            if g.visual.material.baseColorTexture is not None and alphaMode != 0
            else torch.tensor([])
        )  # [H, W]
        normalTexture = (
            torch.tensor(np.array(g.visual.material.normalTexture.convert("RGB"))[..., :3], dtype=torch.uint8)
            if g.visual.material.normalTexture is not None
            else torch.tensor([])
        )  # [H, W, 3]
...
def blender_dump_to_volumetric_attr(
    ...
    # Auto adjust aabb
    if aabb is None:
        min_xyz = np.min([object["vertices"].min(axis=0) for object in dump["objects"]], axis=0)
        max_xyz = np.max([object["vertices"].max(axis=0) for object in dump["objects"]], axis=0)
    ...
    for material in dump["materials"]:
        ...
        if material["baseColorTexture"] is not None:
            baseColorTexture, baseColorTextureFilter, baseColorTextureWrap = load_texture(material["baseColorTexture"])
            assert (
                baseColorTexture.shape[2] == 3
            ), f"Base color texture must have 3 channels, but got {baseColorTexture.shape[2]}"
        else:
        ...
        if material["metallicTexture"] is not None:
            metallicTexture, metallicTextureFilter, metallicTextureWrap = load_texture(material["metallicTexture"])
            assert (
                metallicTexture.dim() == 2
            ), f"Metallic roughness texture must have 2 dimensions, but got {metallicTexture.dim()}"
        else:
        ...
        if material["roughnessTexture"] is not None:
            roughnessTexture, roughnessTextureFilter, roughnessTextureWrap = load_texture(material["roughnessTexture"])
            assert (
                roughnessTexture.dim() == 2
            ), f"Metallic roughness texture must have 2 dimensions, but got {roughnessTexture.dim()}"
        else:
        ...
        if material["alphaTexture"] is not None:
            alphaTexture, alphaTextureFilter, alphaTextureWrap = load_texture(material["alphaTexture"])
    ...
    for object in dump["objects"]:
        triangles = torch.tensor(object["vertices"][object["faces"]], dtype=torch.float32).reshape(-1, 3, 3) - aabb[0].reshape(
            1, 1, 3
        )
        normails = torch.tensor(object["normals"], dtype=torch.float32)
        uvs = (
            torch.tensor(object["uvs"], dtype=torch.float32)
            if object["uvs"] is not None
            else torch.zeros(triangles.shape[0], 3, 2, dtype=torch.float32)
        )
