...
def render_frames(sample, extrinsics, intrinsics, options={}, verbose=True, **kwargs):
    ...
    for j, (extr, intr) in tqdm(
        enumerate(zip(extrinsics, intrinsics)), total=len(extrinsics), desc="Rendering", disable=not verbose
    ):
...
def render_snapshot(
    samples, resolution=512, bg_color=(0, 0, 0), offset=(-16 / 180 * np.pi, 20 / 180 * np.pi), r=10, fov=8, nviews=4, **kwargs
):
