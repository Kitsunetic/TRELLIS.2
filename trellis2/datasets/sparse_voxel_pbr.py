    ...
    def read_mesh_with_texture(self, root, instance):
        ...
        for mat in dump["materials"]:
            materials.append(
                PbrMaterial(
                    base_color_texture=(
                        self._texture_from_dump(mat["baseColorTexture"]) if mat["baseColorTexture"] is not None else None
                    ),
                    base_color_factor=mat["baseColorFactor"],
                    metallic_texture=(
                        self._texture_from_dump(mat["metallicTexture"]) if mat["metallicTexture"] is not None else None
                    ),
                    metallic_factor=mat["metallicFactor"],
                    roughness_texture=(
                        self._texture_from_dump(mat["roughnessTexture"]) if mat["roughnessTexture"] is not None else None
                    ),
        ...
        for obj in dump["objects"]:
            ...
            uv_coords.append(
                obj["uvs"] if obj["uvs"] is not None else np.zeros((obj["faces"].shape[0], 3, 2), dtype=np.float32)
            )
