    ...
    def render(
            ...
            for type in return_types:
                if type == "mask":
                    ...
                    if isinstance(mesh, MeshWithVoxel):
                        ...
                        ret_dict[type] = torch.zeros(
                            (mesh.vertex_attrs.shape[-1], resolution, resolution), dtype=torch.float32, device=self.device
                        )
            return ret_dict
        ...
        if chunk_size is None:
            ...
            if clamp_barycentric_coords:
                ...
                rast[..., :2] /= torch.where(
                    rast[..., :2].sum(dim=-1, keepdim=True) > 1,
                    rast[..., :2].sum(dim=-1, keepdim=True),
                    torch.ones_like(rast[..., :2]),
                )
            for type in return_types:
                img = None
                if type == "mask":
                    ...
                    img = dr.interpolate(
                        ...
                        rast,
                        torch.arange(face_normal.shape[0], dtype=torch.int, device=self.device)
                        .unsqueeze(1)
                        .repeat(1, 3)
                        .contiguous(),
                    )[0]
                        ...
                        for id, mat in enumerate(mesh.materials):
                            ...
                            if mat.base_color_texture is not None:
                                base_color = dr.texture(
                                    ...
                                    filter_mode=(
                                        ...
                                        if mat.base_color_texture.filter_mode == TextureFilterMode.LINEAR
                                        else "nearest"
                                    ),
                                    boundary_mode=(
                                        "clamp" if mat.base_color_texture.wrap_mode == TextureWrapMode.CLAMP_TO_EDGE else "wrap"
                                    ),
                                )[0]
                            ...
                            if mat.metallic_texture is not None:
                                metallic = dr.texture(
                                    ...
                                    filter_mode=(
                                        "linear-mipmap-linear"
                                        if mat.metallic_texture.filter_mode == TextureFilterMode.LINEAR
                                        else "nearest"
                                    ),
                                    boundary_mode=(
                                        "clamp" if mat.metallic_texture.wrap_mode == TextureWrapMode.CLAMP_TO_EDGE else "wrap"
                                    ),
                                )[0]
                            ...
                            if mat.roughness_texture is not None:
                                roughness = dr.texture(
                                    ...
                                    filter_mode=(
                                        "linear-mipmap-linear"
                                        if mat.roughness_texture.filter_mode == TextureFilterMode.LINEAR
                                        else "nearest"
                                    ),
                                    boundary_mode=(
                                        "clamp" if mat.roughness_texture.wrap_mode == TextureWrapMode.CLAMP_TO_EDGE else "wrap"
                                    ),
                                )[0]
                            ...
                            if mat.alpha_mode == AlphaMode.OPAQUE:
                                ...
                                if mat.alpha_texture is not None:
                                    alpha = dr.texture(
                                        ...
                                        filter_mode=(
                                            "linear-mipmap-linear"
                                            if mat.alpha_texture.filter_mode == TextureFilterMode.LINEAR
                                            else "nearest"
                                        ),
                                        boundary_mode=(
                                            "clamp" if mat.alpha_texture.wrap_mode == TextureWrapMode.CLAMP_TO_EDGE else "wrap"
                                        ),
                                    )[0]
            ...
            for i in range(0, faces.shape[0], chunk_size):
                ...
                rast, rast_db = dr.rasterize(self.glctx, vertices_clip, faces_chunk, (resolution * ssaa, resolution * ssaa))
                ...
                for type in return_types:
                    ...
                    if type == "mask":
                        ...
                        img = dr.interpolate(
                            face_normal_chunk.unsqueeze(0),
                            rast,
                            torch.arange(face_normal_chunk.shape[0], dtype=torch.int, device=self.device)
                            .unsqueeze(1)
                            .repeat(1, 3)
                            .contiguous(),
                        )[0]
                            ...
                            for id, mat in enumerate(mesh.materials):
                                ...
                                if mat.base_color_texture is not None:
                                    base_color = dr.texture(
                                        ...
                                        boundary_mode=(
                                            "clamp"
                                            if mat.base_color_texture.wrap_mode == TextureWrapMode.CLAMP_TO_EDGE
                                            else "wrap"
                                        ),
                                    )[0]
                                    imgs["base_color"] += base_color * mat.base_color_factor * mat_mask
                                ...
                                if mat.metallic_texture is not None:
                                    metallic = dr.texture(
                                        ...
                                        boundary_mode=(
                                            "clamp"
                                            if mat.metallic_texture.wrap_mode == TextureWrapMode.CLAMP_TO_EDGE
                                            else "wrap"
                                        ),
                                    )[0]
                                    imgs["metallic"] += metallic * mat.metallic_factor * mat_mask
                                ...
                                if mat.roughness_texture is not None:
                                    roughness = dr.texture(
                                        ...
                                        boundary_mode=(
                                            "clamp"
                                            if mat.roughness_texture.wrap_mode == TextureWrapMode.CLAMP_TO_EDGE
                                            else "wrap"
                                        ),
                                    )[0]
                                    imgs["roughness"] += roughness * mat.roughness_factor * mat_mask
                                ...
                                if mat.alpha_mode == AlphaMode.OPAQUE:
                                    ...
                                    if mat.alpha_texture is not None:
                                        alpha = dr.texture(
                                            ...
                                            boundary_mode=(
                                                "clamp"
                                                if mat.alpha_texture.wrap_mode == TextureWrapMode.CLAMP_TO_EDGE
                                                else "wrap"
                                            ),
                                        )[0]
                                        if mat.alpha_mode == AlphaMode.MASK:
        ...
        for type in return_types:
            ...
            if ssaa > 1:
                img = F.interpolate(
                    img.permute(0, 3, 1, 2), (resolution, resolution), mode="bilinear", align_corners=False, antialias=True
                )
                img = img.squeeze()
            else:
