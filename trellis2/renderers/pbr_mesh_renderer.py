...
def screen_space_ambient_occlusion(
    ...
    y_grid, x_grid = torch.meshgrid(
        (torch.arange(H, device=device) + 0.5) / H * 2 - 1, (torch.arange(W, device=device) + 0.5) / W * 2 - 1, indexing="ij"
    )
...
class PbrMeshRenderer:
    ...
    def render(
        ...
        rays_o, rays_d = utils3d.torch.get_image_rays(extrinsics, intrinsics, resolution * ssaa, resolution * ssaa)
        ...
        with dr.DepthPeeler(self.glctx, vertices_clip, faces, (resolution * ssaa, resolution * ssaa)) as peeler:
            for _ in range(self.rendering_options["peel_layers"]):
                ...
                gb_normal = torch.where(torch.sum(gb_normal * (pos - rays_o), dim=-1, keepdim=True) > 0, -gb_normal, gb_normal)
        ...
        # Ambient occulusion
        f_occ = screen_space_ambient_occlusion(depth, normal, perspective, intensity=1.5)
        shaded *= 1 - f_occ
