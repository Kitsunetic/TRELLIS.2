...
class SparseSpatial2Channel(nn.Module):
    ...
    def forward(self, x: SparseTensor) -> SparseTensor:
        ...
        new_feats = torch.zeros(
            new_coords.shape[0] * self.factor**DIM, x.feats.shape[1], device=x.feats.device, dtype=x.feats.dtype
        )
        ...
        out = SparseTensor(
            ...
            new_coords,
            None if x._shape is None else torch.Size([x._shape[0], x._shape[1] * self.factor**DIM]),
        )
...
class SparseChannel2Spatial(nn.Module):
    ...
    def forward(self, x: SparseTensor, subdivision: Optional[SparseTensor] = None) -> SparseTensor:
        ...
        if cache is None:
            if subdivision is None:
                ...
                idx = torch.repeat_interleave(
                    torch.arange(x.coords.shape[0], device=x.device), N_leaf, dim=0, output_size=subidx.shape[0]
                )
        else:
            new_coords, idx, subidx = cache
        ...
        out = SparseTensor(
            new_feats, new_coords, None if x._shape is None else torch.Size([x._shape[0], x._shape[1] // self.factor**DIM])
        )
        out._scale = tuple([s / self.factor for s in x._scale])
