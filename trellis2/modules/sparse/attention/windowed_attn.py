...
def calc_window_partition(
    ...
    if config.ATTN == "xformers":
        ...
        attn_func_args = {"attn_bias": xops.fmha.BlockDiagonalMask.from_seqlens(seq_lens)}
...
def sparse_windowed_scaled_dot_product_self_attention(
    ...
    if config.DEBUG:
        ...
        for i in range(len(seq_lens)):
            ...
            assert (
                seq_coords[:, 1:].max(dim=0).values - seq_coords[:, 1:].min(dim=0).values < window_size
            ).all(), f"SparseWindowedScaledDotProductSelfAttention: window size exceeded"
            start += seq_lens[i]
...
def sparse_windowed_scaled_dot_product_cross_attention(
    ...
    if q_serialization_spatial_cache is None:
        ...
        q.register_spatial_cache(
            q_serialization_spatial_cache_name, (q_fwd_indices, q_bwd_indices, q_seq_lens, q_attn_func_args)
        )
    else:
    ...
    if kv_serialization_spatial_cache is None:
        kv_fwd_indices, kv_bwd_indices, kv_seq_lens, kv_attn_func_args = calc_window_partition(
            kv, kv_window_size, kv_shift_window
        )
        kv.register_spatial_cache(
            kv_serialization_spatial_cache_name, (kv_fwd_indices, kv_bwd_indices, kv_seq_lens, kv_attn_func_args)
        )
    else:
