    ...
    def cum_seqlen(self) -> torch.LongTensor:
        if "cum_seqlen" not in self._cache:
            self._cache["cum_seqlen"] = torch.cat(
                [torch.tensor([0], dtype=torch.long, device=self.device), self.seqlen.cumsum(dim=0)], dim=0
            )
...
class SparseTensor(VarLenTensor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if method_id == 0:
            ...
            if config.CONV == "torchsparse":
                ...
                self.data = self.SparseTensorData(
                    feats.reshape(feats.shape[0], -1), coords, spatial_shape[1:], spatial_shape[0], **kwargs
                )
    ...
    def cum_seqlen(self) -> torch.LongTensor:
        ...
        if cum_seqlen is None:
            cum_seqlen = torch.cat([torch.tensor([0], dtype=torch.long, device=self.device), self.seqlen.cumsum(dim=0)], dim=0)
