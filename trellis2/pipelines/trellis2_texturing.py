...
class Trellis2TexturingPipeline(Pipeline):
    ...
    model_names_to_load = ["shape_slat_encoder", "tex_slat_decoder", "tex_slat_flow_model_512", "tex_slat_flow_model_1024"]
    ...
    def postprocess_mesh(
        ...
        uvs_torch = torch.cat(
            [uvs_torch * 2 - 1, torch.zeros_like(uvs_torch[:, :1]), torch.ones_like(uvs_torch[:, :1])], dim=-1
        ).unsqueeze(0)
        rast, _ = dr.rasterize(
            ctx,
            uvs_torch,
    ...
    def run(
        ...
        tex_slat = self.sample_tex_slat(cond, tex_model, shape_slat, tex_slat_sampler_params)
