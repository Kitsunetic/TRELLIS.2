...
class SpaceControlPipeline(Pipeline):
    ...
    def run(
        ...
        # ==========================================
        # [추가됨] SpaceControl: 이후 단계를 위해 정리
        # ==========================================
        # shape_slat과 tex_slat 모델은 "control" 키를 받으면 에러를 낼 수 있으므로 안전하게 제거합니다.
        if "control" in cond_512:
            cond_512.pop("control")
        if cond_1024 is not None and "control" in cond_1024:
            cond_1024.pop("control")
        # ==========================================
    ...
    def encode_spatial_control(self, spatial_control_path: str) -> torch.Tensor:
        """
        Encode the spatial control for SpaceControl.
        """
        # 1. 3D 메쉬를 복셀화 (V1의 utils 재사용)
        spatial_control = voxelize_sq_francis(spatial_control_path).to(device=self.device)

        # 2. VRAM 관리를 위해 인코더를 잠시 디바이스에 올림
        if self.low_vram:
            self.models["sparse_structure_encoder"].to(self.device)
        ...
        # 4. 인코더를 다시 CPU로 내림
        if self.low_vram:
            self.models["sparse_structure_encoder"].cpu()
