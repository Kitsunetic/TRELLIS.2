...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(opt.pbr_voxel_root, f"pbr_voxels_{opt.resolution}", "metadata.csv")):
        metadata = metadata.combine_first(
            pd.read_csv(os.path.join(opt.pbr_voxel_root, f"pbr_voxels_{opt.resolution}", "metadata.csv")).set_index("sha256")
        )
    if os.path.exists(os.path.join(opt.pbr_latent_root, "pbr_latents", latent_name, "metadata.csv")):
        metadata = metadata.combine_first(
            pd.read_csv(os.path.join(opt.pbr_latent_root, "pbr_latents", latent_name, "metadata.csv")).set_index("sha256")
        )
        ...
        def loader(sha256):
            try:
                ...
                coords, attr = o_voxel.io.read_vxz(
                    os.path.join(opt.pbr_voxel_root, f"pbr_voxels_{opt.resolution}", f"{sha256}.vxz"), num_threads=4
                )
    ...
    records.to_csv(
        os.path.join(opt.pbr_latent_root, "pbr_latents", latent_name, "new_records", f"part_{opt.rank}.csv"), index=False
    )
