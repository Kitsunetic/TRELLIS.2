...
if __name__ == "__main__":
    ...
    parser.add_argument("--pbr_dump_root", type=str, default=None, help="Directory to load mesh dumps")
    parser.add_argument("--pbr_voxel_root", type=str, default=None, help="Directory to save voxelized pbr attributes")
    parser.add_argument(
    ...
    for res in opt.resolution:
        if os.path.exists(os.path.join(opt.pbr_voxel_root, f"pbr_voxels_{res}", "metadata.csv")):
            pbr_voxel_metadata = pd.read_csv(os.path.join(opt.pbr_voxel_root, f"pbr_voxels_{res}", "metadata.csv")).set_index(
                "sha256"
            )
            pbr_voxel_metadata = pbr_voxel_metadata.rename(
                columns={"pbr_voxelized": f"pbr_voxelized_{res}", "num_pbr_voxels": f"num_pbr_voxels_{res}"}
            )
    ...
    pbr_voxelized = dataset_utils.foreach_instance(
        metadata, None, func, max_workers=opt.max_workers, no_file=True, desc="Voxelizing"
    )
    ...
    for res in opt.resolution:
        if f"pbr_voxelized_{res}" in pbr_voxelized.columns:
            ...
            if len(pbr_voxel_metadata) > 0:
                ...
                pbr_voxel_metadata = pbr_voxel_metadata.rename(
                    columns={f"pbr_voxelized_{res}": "pbr_voxelized", f"num_pbr_voxels_{res}": "num_pbr_voxels"}
                )
                pbr_voxel_metadata.to_csv(
                    os.path.join(opt.pbr_voxel_root, f"pbr_voxels_{res}", "new_records", f"part_{opt.rank}.csv"), index=False
                )
