...
def try_extract_image(link, expected_channel="RGB"):
    ...
    if expected_channel in ["R", "G", "B"]:
        ...
        assert (
            link.from_node.type == "SEPARATE_COLOR" and link.from_node.mode == "RGB"
        ...
        assert (
            sep_node.inputs[0].is_linked and sep_node.inputs[0].links[0].from_node.type == "TEX_IMAGE"
...
def try_extract_factor(link, mode="color"):
    ...
    if mode == "color":
        if link.from_node.type == "MIX":
            ...
            assert (
                mix_node.data_type == "RGBA" and mix_node.blend_type == "MULTIPLY"
            ), f"Material is not supported, {mix_node.data_type}, {mix_node.blend_type}"
            assert (
                not mix_node.inputs["Factor"].is_linked and mix_node.inputs["Factor"].default_value == 1.0
            ), "Material is not supported"
...
def main(arg):
                    ...
                    if node.type == "MATH":
                        if node.operation == "ROUND":
                            ...
                            assert (
                                ...
                                and node.inputs[1].is_linked
                                and node.inputs[1].links[0].from_node.type == "MATH"
                                and node.inputs[1].links[0].from_node.operation == "LESS_THAN"
                            ), "Material is not supported"
    ...
    for obj in scene.objects:
        ...
        pack["normals"] = np.array(
            [[eval_mesh.loops[i].normal for i in poly.loop_indices] for poly in eval_mesh.polygons], dtype=np.float32
        )  # (F, 3, 3)

        if eval_mesh.uv_layers.active is not None:
            pack["uvs"] = np.array(
                [[eval_mesh.uv_layers.active.data[i].uv for i in poly.loop_indices] for poly in eval_mesh.polygons],
                dtype=np.float32,
            )  # (F, 3, 2)
        ...
        pack["mat_ids"] = np.array(
            [
                (
                    ...
                    if len(obj.material_slots) > 0 and obj.material_slots[poly.material_index].material is not None
                    else -1
                )
