...
if __name__ == "__main__":
    ...
    parser.add_argument("--dual_grid_root", type=str, default=None, help="Directory to save the dual grids")
    ...
    parser.add_argument("--resolution", type=int, default=1024, help="Sparse voxel resolution")
    parser.add_argument(
        "--enc_pretrained",
        type=str,
    ...
    if os.path.exists(os.path.join(opt.dual_grid_root, f"dual_grid_{opt.resolution}", "metadata.csv")):
        metadata = metadata.combine_first(
            pd.read_csv(os.path.join(opt.dual_grid_root, f"dual_grid_{opt.resolution}", "metadata.csv")).set_index("sha256")
        )
    if os.path.exists(os.path.join(opt.shape_latent_root, "shape_latents", latent_name, "metadata.csv")):
        metadata = metadata.combine_first(
            pd.read_csv(os.path.join(opt.shape_latent_root, "shape_latents", latent_name, "metadata.csv")).set_index("sha256")
        )
        ...
        def loader(sha256):
            try:
                coords, attr = o_voxel.io.read_vxz(
                    os.path.join(opt.dual_grid_root, f"dual_grid_{opt.resolution}", f"{sha256}.vxz"), num_threads=4
                )
    ...
    records.to_csv(
        os.path.join(opt.shape_latent_root, "shape_latents", latent_name, "new_records", f"part_{opt.rank}.csv"), index=False
    )
