...
def _render_cond(file_path, metadatum, root, num_cond_views):
    ...
    args = [
        ...
        "1024",
        "--cond_output_folder",
        os.path.join(root, "renders_cond", sha256),
        "--engine",
        "CYCLES",
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--render_cond_root", type=str, default=None, help="Directory to save the mesh dumps")
    parser.add_argument(
    ...
    parser.add_argument("--num_cond_views", type=int, default=16, help="Number of conditional views to render")
    ...
    if os.path.exists(os.path.join(opt.render_cond_root, "renders_cond", "metadata.csv")):
        metadata = metadata.combine_first(
            pd.read_csv(os.path.join(opt.render_cond_root, "renders_cond", "metadata.csv")).set_index("sha256")
        )
    ...
    cond_rendered = dataset_utils.foreach_instance(
        metadata, opt.render_cond_root, func, max_workers=opt.max_workers, desc="Rendering objects"
    )
