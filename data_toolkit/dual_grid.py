...
if __name__ == "__main__":
    ...
    parser.add_argument("--mesh_dump_root", type=str, default=None, help="Directory to load mesh dumps")
    parser.add_argument("--dual_grid_root", type=str, default=None, help="Directory to save dual grids")
    parser.add_argument(
    ...
    for res in opt.resolution:
        if os.path.exists(os.path.join(opt.dual_grid_root, f"dual_grid_{res}", "metadata.csv")):
            dual_grid_metadata = pd.read_csv(os.path.join(opt.dual_grid_root, f"dual_grid_{res}", "metadata.csv")).set_index(
                "sha256"
            )
            dual_grid_metadata = dual_grid_metadata.rename(
                columns={"dual_grid_converted": f"dual_grid_converted_{res}", "dual_grid_size": f"dual_grid_size_{res}"}
            )
    ...
    dual_grids = dataset_utils.foreach_instance(
        metadata, None, func, max_workers=opt.max_workers, no_file=True, desc="Dual griding"
    )
    ...
    for res in opt.resolution:
        if f"dual_grid_converted_{res}" in dual_grids.columns:
            ...
            if len(dual_grid_metadata) > 0:
                ...
                dual_grid_metadata = dual_grid_metadata.rename(
                    columns={f"dual_grid_converted_{res}": "dual_grid_converted", f"dual_grid_size_{res}": "dual_grid_size"}
                )
                dual_grid_metadata.to_csv(
                    os.path.join(opt.dual_grid_root, f"dual_grid_{res}", "new_records", f"part_{opt.rank}.csv"), index=False
                )
