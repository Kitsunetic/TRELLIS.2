...
if __name__ == "__main__":
    ...
    parser.add_argument("--ss_latent_root", type=str, default=None, help="Directory to save the shape latent files")
    parser.add_argument(
    ...
    parser.add_argument("--resolution", type=int, default=64, help="Sparse voxel resolution")
    parser.add_argument("--shape_latent_name", type=str, default=None, help="Name of the shape latent files")
    parser.add_argument(
        "--enc_pretrained",
        type=str,
    ...
    parser.add_argument("--enc_model", type=str, help="Encoder model. if specified, use this model instead of pretrained model")
    ...
    if os.path.exists(os.path.join(opt.shape_latent_root, "shape_latents", opt.shape_latent_name, "metadata.csv")):
        metadata = metadata.combine_first(
            pd.read_csv(os.path.join(opt.shape_latent_root, "shape_latents", opt.shape_latent_name, "metadata.csv")).set_index(
                "sha256"
            )
        )
    if os.path.exists(os.path.join(opt.ss_latent_root, "ss_latents", latent_name, "metadata.csv")):
        metadata = metadata.combine_first(
            pd.read_csv(os.path.join(opt.ss_latent_root, "ss_latents", latent_name, "metadata.csv")).set_index("sha256")
        )
    ...
    with ThreadPoolExecutor(max_workers=32) as loader_executor, ThreadPoolExecutor(max_workers=32) as saver_executor:
        ...
        def loader(sha256):
            try:
                coords = np.load(os.path.join(opt.shape_latent_root, "shape_latents", opt.shape_latent_name, f"{sha256}.npz"))[
                    "coords"
                ]
    ...
    records.to_csv(
        os.path.join(opt.ss_latent_root, "ss_latents", latent_name, "new_records", f"part_{opt.rank}.csv"), index=False
    )
